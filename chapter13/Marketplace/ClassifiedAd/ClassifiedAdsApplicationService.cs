...
using Marketplace.ClassifiedAds.Domain.ClassifiedAd;
        ...
        public async Task Handle(object command)
        {
            switch (command)
            {
                case Contracts.V1.Create cmd:
                    if (await _store.Exists<ClassifiedAds.Domain.ClassifiedAd.ClassifiedAd, ClassifiedAdId>(
                        new ClassifiedAdId(cmd.Id)))
                        throw new InvalidOperationException($"Entity with id {cmd.Id} already exists");

                    var classifiedAd = new ClassifiedAds.Domain.ClassifiedAd.ClassifiedAd(
                        new ClassifiedAdId(cmd.Id),
                        new UserId(cmd.OwnerId));

                    await _store.Save<ClassifiedAds.Domain.ClassifiedAd.ClassifiedAd, ClassifiedAdId>(classifiedAd);
                    break;

                case Contracts.V1.SetTitle cmd:
                    await this.HandleUpdate<ClassifiedAds.Domain.ClassifiedAd.ClassifiedAd, ClassifiedAdId>(
