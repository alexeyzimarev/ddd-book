...
using Marketplace.ClassifiedAds.Domain.UserProfile;
        ...
        public async Task Handle(object command)
        {
            switch (command)
            {
                case Contracts.V1.RegisterUser cmd:
                    if (await _store.Exists<ClassifiedAds.Domain.UserProfile.UserProfile, UserId>(new UserId(cmd.UserId)))
                        throw new InvalidOperationException($"Entity with id {cmd.UserId} already exists");

                    var userProfile = new ClassifiedAds.Domain.UserProfile.UserProfile(
                        new UserId(cmd.UserId),
                        FullName.FromString(cmd.FullName),
                        DisplayName.FromString(cmd.DisplayName, _checkText));

                    await _store.Save<ClassifiedAds.Domain.UserProfile.UserProfile, UserId>(userProfile);
                    break;
                case Contracts.V1.UpdateUserFullName cmd:
                    await this.HandleUpdate<ClassifiedAds.Domain.UserProfile.UserProfile, UserId>(_store, new UserId(cmd.UserId),
